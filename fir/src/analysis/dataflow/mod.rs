...
use crate::instruction::{Instr, ValueData};

pub mod lattice;
pub mod concrete_value;

type InstrValue = crate::instruction::Value;

#[derive(Default)]
pub struct ForwardAnalysis< V: lattice::Value> {
    pub (crate) state: FxHashMap<BasicBlock, FxHashMap<InstrValue, lattice::Element<V>>>,
}
...
impl< V: lattice::Value> ForwardAnalysis< V> {
    ...
    pub fn run<H: FnMut(&mut Self, &mut FunctionData, BasicBlock, Instr) -> bool>(&mut self, function: &mut FunctionData, mut handler: H) -> usize {
