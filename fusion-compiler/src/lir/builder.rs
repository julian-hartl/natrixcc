...
use crate::lir::{BasicBlock, BasicBlockIdx, ConstOp, Function, Instruction, InstructionKind, LIR, Operand, OperandKind, Place, PlaceIdx, Terminator, Type};
use crate::mir;
use crate::mir::{Binop, InstructionIdx, MIR, Value};
...
impl<'mir> LIRBuilder<'mir> {
    ...
    pub fn build(mut self) -> LIR {
        for (mir_function_idx, mir_function) in self.mir.functions.indexed_iter() {
            ...
            let function_idx = self.lir.functions.push(
                Function {
            ...
            for bb_idx in mir_function.basic_blocks.iter().copied() {
                ...
                for instruction_idx in mir_bb.instructions.iter().copied() {
                    ...
                    let instruction = match &mir_instruction.kind {
                        mir::InstructionKind::Binary { lhs, rhs, operator } => {
                            ...
                            match operator {
                                Binop::Add => InstructionKind::Add { target: self.get_referencing_place(instruction_idx), lhs, rhs },
                                Binop::Sub => InstructionKind::Sub { target: self.get_referencing_place(instruction_idx), lhs, rhs },
                                Binop::Gt => InstructionKind::Gt { target: self.get_referencing_place(instruction_idx), lhs, rhs },
                                _ => todo!("Unsupported binary operator {:?}", operator),
                            }
                        }
                        mir::InstructionKind::Unary { .. } => todo!(),
                        mir::InstructionKind::Value(value) => {
                            InstructionKind::AllocInit {
                    ...
                    self.current_basic_block().instructions.push(
                        Instruction {
                            kind: instruction,
                        }
                    );
                ...
                function.basic_blocks.push(self.current_bb.expect("No current basic block"));
            }
        }
        self.lir
    }
    ...
    fn start_basic_block(&mut self) -> BasicBlockIdx {
        let bb_idx = self.lir.basic_blocks.push(
            BasicBlock::default()
        );
    ...
    fn current_basic_block(&mut self) -> &mut BasicBlock {
        self.lir.basic_blocks.get_mut(self.current_bb.expect("No current basic block"))
    }

    fn get_current_function(&self) -> &mir::Function {
        self.mir.functions.get(self.current_mir_function_idx.expect("No current function"))
    }

    fn get_referencing_place(&mut self, instruction_idx: InstructionIdx) -> PlaceIdx {
        let instruction = self.get_current_function().instructions.get(instruction_idx);
        let ty = instruction.ty.into();
        let aliased_var = self.get_current_function().local_aliases.get(&instruction_idx).copied();
        match aliased_var {
            None => {
                match self.instruction_to_place.get(&instruction_idx) {
                    None => {
            ...
            Some(aliased_var) => {
                match self.var_to_place.get(&aliased_var) {
                    None => {
                        let place = self.create_place(ty);
    ...
    fn create_place(&mut self, ty: Type) -> PlaceIdx {
        self.lir.places.push_with_index(
            |idx|
                Place {
                    ty,
                    idx,
