...
pub struct GlobalScope {
    pub variables: IdxVec<VariableIdx, Variable>,
...
impl Scopes {
    ...
    fn _declare_variable(&mut self, identifier: &str, ty: Type, is_global: bool) -> VariableIdx {
        ...
        self.global_scope
            .declare_variable(identifier, ty, is_global, is_shadowing)
    }
    ...
    fn surrounding_function(&self) -> Option<&Function> {
        return self
            .surrounding_function_idx()
            .map(|function| self.global_scope.functions.get(function));
    }
...
impl ASTVisitor for Resolver {
    ...
    fn visit_call_expression(&mut self, ast: &mut Ast, call_expression: &CallExpr, expr: &Expr) {
        ...
        let ty = match function {
            ...
            Some(function_idx) => {
                let function = self.scopes.global_scope.functions.get(function_idx);
                ast.set_function(expr.id, function_idx);
    ...
    fn visit_binary_expression(
        ...
        let ty =
            self.resolve_binary_expression(ast, &left, &right, &binary_expression.operator.kind);
        ast.set_type(expr.id, ty);
    }
    ...
    fn visit_parenthesized_expression(
        ...
        let expression = ast.query_expr(parenthesized_expression.inner);
...
impl CompilationUnit {
    pub fn compile(input: &str) -> Result<CompilationUnit, DiagnosticsBagCell> {
        ...
        Self::check_diagnostics(&text, &diagnostics_bag)
            .map_err(|_| Rc::clone(&diagnostics_bag))?;
    ...
    pub fn run(&mut self) {
        ...
        if let Some(function) = main_function_ref {
            ...
            for stmt in &*function.body {
                eval.visit_statement(&mut self.ast, *stmt);
            }
        } else {
    ...
    fn check_diagnostics(
        ...
        if diagnostics_binding.diagnostics.len() > 0 {
            let diagnostics_printer =
                DiagnosticsPrinter::new(&text, &diagnostics_binding.diagnostics);
