...
use crate::analysis::dataflow::{Analysis, DFState, InstrWalker};
...
impl<'a, A: Analysis> BackwardAnalysisRunner<'a, A> {
    ...
    pub fn next_bb(&mut self) -> Option<(BasicBlockId, BAInstrWalker<A>)> {
        ...
        let bb_state = self.state.create(bb_id, self.function.cfg.successors(bb_id));
        ...
        for pred in self.function.cfg.predecessors(bb_id) {
            ...
            let all_succs_visited = succs.all(|succ| {
                self.visited.contains(succ)
            });
...
impl<'a, 'b, A: Analysis> InstrWalker<A::V> for BAInstrWalker<'a, 'b, A> {
    fn walk<H>(mut self, mut h: H) where H: FnMut(&mut Instr, &A::V) {
