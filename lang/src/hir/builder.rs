...
use crate::ast::{Ast, ExprId, ExprKind, ItemKind, StmtId, StmtKind};
use crate::compilation_unit::{GlobalScope, VariableIdx};
use crate::hir::{HIR, HIRExpr, HIRExprKind, HIRStmt, HIRStmtKind, Type};
...
impl HIRBuilder {
    ...
    fn build_stmt(&self, stmt_id: StmtId, ast: &Ast, global_scope: &mut GlobalScope, ctx: &mut Ctx) {
        ...
        let kind = match &stmt.kind {
            ...
            StmtKind::Return(return_stmt) => {
                let expr = return_stmt.return_value.as_ref().copied().map(|expr_id| {
                    self.build_expr(expr_id, ast, global_scope, ctx)
                }).unwrap_or(HIRExpr {
    ...
    fn build_expr(&self, expr_id: ExprId, ast: &Ast, global_scope: &mut GlobalScope, ctx: &mut Ctx) -> HIRExpr {
        let expr = ast.query_expr(expr_id);
        let kind = match &expr.kind {
            ExprKind::Number(number_expr) => {
                HIRExprKind::Number(number_expr.number)
            }
            ...
            ExprKind::Parenthesized(parenthesized_expr) => {
                self.build_expr(parenthesized_expr.inner, ast, global_scope, ctx).kind
            }
            ExprKind::Variable(variable_expr) => {
                HIRExprKind::Var(variable_expr.variable_idx)
            }
            ExprKind::Boolean(boolean_expr) => {
                HIRExprKind::Bool(boolean_expr.value)
            }
            ExprKind::Call(call_expr) => {
                let arguments = call_expr.arguments.iter().map(|expr_id| {
                    self.build_expr(*expr_id, ast, global_scope, ctx)
                }).collect();
                HIRExprKind::Call {
            ...
            ExprKind::Block(block_expr) => {
                ...
                let expr_kind = if matches!(expr.ty, Type::Void) {
                    ...
                    let temp_variable_idx = self.declare_next_temp_var(global_scope, expr.ty.clone());
                    ctx.stmts.push(HIRStmt {
    ...
    fn declare_next_temp_var(&self, global_scope: &mut GlobalScope, ty: Type) -> VariableIdx {
        global_scope.declare_variable(
            &self.next_temp_variable(),
            ty,
            false,
            false,
        )
