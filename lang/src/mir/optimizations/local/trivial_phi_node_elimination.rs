...
impl LocalMIRPass for TrivialPhiNodeElimination {
    fn run_on_basic_block(
        ...
        for instruction_idx in bb.instructions.iter().copied() {
            let instruction = mir.functions[function_idx]
                .instructions
            ...
            if let InstructionKind::Phi(phi) = &instruction.kind {
                ...
                if let Some(referenced_instruction) = referenced_instruction {
                    tracing::info!(
                        "Found a trivial phi node in {} with same operands, removing it",
                        bb_idx
                    );
                    changes += 1;
                    instruction.kind =
                        InstructionKind::Value(Value::InstructionRef(*referenced_instruction));
                }
            }
        }
        changes
